<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <form v-else @submit.prevent="handleSubmit" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        ...
        <div class="mb-6">
          ...
          <div v-if="form.image && !uploadingImage" class="mb-4">
            <p class="text-sm font-medium text-gray-700 mb-2">Current Image:</p>
            <img :src="form.image" alt="Current image preview" class="w-full max-w-sm rounded-lg border border-gray-200 shadow-sm" >
          </div>
          ...
          >
          <p class="text-sm text-gray-500 mt-1">Upload a new image to replace the one above (will be converted to WebP)</p>
...
const { getPostById, updatePost, generateSlug } = useBlog()
...
const form = reactive({
  ...
  category: '',
  excerpt: '',
  content: '', // This will hold the HTML string from the editor
  image: '',
  published: true
})
...
const uploadToSupabase = async (file: File) => {
  const bucketName = 'posts' // IMPORTANT: Replace with your bucket name
...
const handleImageUpload = async (event: Event) => {
  ...
  imageError.value = ''
  form.image = '' // Hide old preview while uploading
  ...
  try {
    ...
    originalImageUrl.value = publicUrl

  } catch (err: any) {
    console.error('Image upload failed:', err)
    imageError.value = err.message || 'Failed to upload image. Please try again.'
    form.image = originalImageUrl.value
  } finally {
    uploadingImage.value = false
  }
}

// handleSubmit (modified with content validation)
const handleSubmit = async () => {
  if (!form.image) {
    error.value = 'A featured image is required. Please upload one.'
    return
  }
...
// onMounted (no change, already correctly loads content)
onMounted(async () => {
  try {
    const postId = route.params.id as string
    ...
    form.content = post.content // This HTML will populate the Quill editor
    form.image = post.image
    form.published = post.published

    originalImageUrl.value = post.image

  } catch (err: any) {
    loadError.value = err.message || 'Failed to load post'
    console.error(err)
  } finally {
    loading.value = false
  }
