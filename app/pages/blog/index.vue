    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div class="mb-10">
        ...
        >
          <div class="w-8 h-8 rounded-full bg-gray-100 group-hover:bg-[#5a912d]/10 flex items-center justify-center mr-2 transition-all duration-300">
            <UIcon name="i-lucide-chevron-left" class="w-4 h-4" aria-hidden="true" />
          </div>
          <span>Back</span>
        </button>
      </div>
      ...
      <div v-else-if="error" class="text-center py-16">
        <div class="inline-flex items-center justify-center w-16 h-16 bg-red-100 rounded-full mb-4">
          <UIcon name="i-lucide-alert-circle" class="w-8 h-8 text-red-600" aria-hidden="true" />
        </div>
        <p class="text-red-600 text-lg font-medium">Failed to load blog posts. Please try again later.</p>
      </div>

      <div v-else-if="!allPosts || allPosts.length === 0" class="text-center py-20">
      ...
      <div v-else>
              ...
              <div class="relative p-6 flex-grow flex flex-col z-20">
                ...
                <div class="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
                  <div class="flex items-center text-xs text-gray-500">
                    <UIcon name="i-lucide-calendar" class="w-3.5 h-3.5 mr-1.5" aria-hidden="true" />
                  ...
                  <div class="flex items-center font-semibold text-[#5a912d] text-sm group-hover:translate-x-1 transition-transform duration-300">
                    <span>Read</span>
                    <UIcon name="i-lucide-arrow-right" class="w-4 h-4 ml-1" aria-hidden="true" />
                  </div>
                </div>
              </div>
        ...
        <div v-if="totalPages > 1" class="mt-16 flex flex-col sm:flex-row justify-between items-center gap-4">
          <button
            ...
            @click="currentPage--"
          >
            <UIcon name="i-lucide-chevron-left" class="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform duration-300" aria-hidden="true" />
            Previous
          </button>
          ...
          >
            Next
            <UIcon name="i-lucide-chevron-right" class="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform duration-300" aria-hidden="true" />
          </button>
        </div>
      </div>
...
// 1. Fetch posts on the server side using useAsyncData
...
// Safely compute total pages handling possible null/undefined values
const totalPages = computed(() => {
  return Math.ceil((allPosts.value?.length || 0) / postsPerPage)
})

// Safely compute paginated posts
